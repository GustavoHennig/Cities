...
class PHPLiveX {
	...
	public function Export($funcList = null){
		...
		foreach($funcArray as $key => $val){
			$val = stripslashes(trim($val));
			if(function_exists($val)){
				if(!in_array($val, $this->FList)){
					$this->FList[] = $val;
				}
			}else{
				$this->Error .= "'" . $val . "' function doesn't exist!";
				?>
	...
	public function ExportObjectMethods($funcList = null){
		...
		foreach($funcArray as $key => $val){
			$val = stripslashes(trim($val));
			if(strpos($val, "->") !== false) $symbol = "->";
			else if(strpos($val, "::") !== false) $symbol = "::";
			else continue;

			$parts = explode($symbol, $val);
	...
	public function ExportObjects($instanceList = ""){
		...
		foreach($instanceArray as $key => $val){
			$val = stripslashes(trim($val));
	 ...
	 *
	 * @param Boolean $includeMainMethods: To include main phplivex methods or not.
	 */
	public function Run($includeMainMethods = true){
		if(!$includeMainMethods){
			$js .= str_replace("\r\n", "", $this->GetFunctions());
			echo $js;
			return;
		}
		...
		$js = $this->CreateJS();
		foreach($this->FList as $key => $val) $js .= str_replace("\r\n", "", $this->CreateFunction($val));
	...
	public function GetFunctions(){
		$html = "";
		foreach($this->FList as $key => $val){
			$html .= $this->CreateFunction($val);
		}
